...
public class Client
{
    ...
    public Action OnPlayerPreSentToGame;
    ...
    {
        PlayerModel = PlayerSpawner.Instance.SpawnPlayer();
        PlayerModel.Init(Id, playerName);

        var cameraController = PlayerModel.gameObject.AddComponent<CameraPositionController>();
        cameraController.Init(Id, PlayerModel.transform);
        
        foreach (var client in Server.Clients.Values)
        {
            if (client.PlayerModel != null)
            {
                if (client.Id != Id)
                {
                    ServerSend.SpawnPlayer(Id, client.PlayerModel);
                }
            }
        }
        ...
        foreach (var client in Server.Clients.Values)
        {
            if (client.PlayerModel != null)
            {
                ServerSend.SpawnPlayer(client.Id, PlayerModel);
            }
        }
    }
    ...
    public void Disconnect()
    {
        ...
        ThreadManager.ExecuteOnMainThread(() =>
        {
            UnityEngine.Object.Destroy(PlayerModel.gameObject);
            PlayerModel = null;
        });
